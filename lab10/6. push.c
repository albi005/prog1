...
ListaElem *vege(ListaElem *p) {
    if (p == NULL) return NULL;
    if (p->kov == NULL) return p;
    return vege(p->kov);
}

ListaElem *lista_vegere_beszur(ListaElem *eleje, int data) {
    ListaElem *new = (ListaElem*)malloc(sizeof(ListaElem));
    new->adat = data;
    new->kov = NULL;

    ListaElem *utolso = vege(eleje);
    if (utolso == NULL) return new;
    utolso->kov = new;
    return eleje;
}
...
int main(void) {
    ...
    eleje = lista_vegere_beszur(eleje, 23);
